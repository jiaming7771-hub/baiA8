...
# 项目若在 Desktop，LaunchAgent 会被 TCC 拦截；因此实际跑的是
# ~/Library/Application Support/a8-pump/runtime 这份同步副本。
set -euo pipefail
...
SYNC_SRC="$ROOT/scripts/sync_live_runtime.sh"
LOG_OUT="/tmp/a8_watchdog.out"
LOG_ERR="/tmp/a8_watchdog.err"

chmod +x "$WATCHDOG_SRC" "$SYNC_SRC"
mkdir -p "$SUPPORT"
...
sleep 3
if curl -sf -m 3 http://127.0.0.1:8000/api/pump/status >/dev/null; then
  echo "✅ LaunchAgent 已安装且 API 就绪"
else
  echo "⚠️ LaunchAgent 已安装，但 API 尚未就绪，看 /tmp/a8_bot.log 与 /tmp/a8_watchdog.err"
fi
echo "   plist: $PLIST"
echo "   watchdog: $WATCHDOG_DST"
echo "   运行目录: $RUNTIME   ← LaunchAgent 只读这里（不读 Desktop）"
echo "   源码目录: $ROOT"
echo "   改代码后请执行: bash scripts/sync_live_runtime.sh && launchctl kickstart -k gui/\$(id -u)/${LABEL}"
